...
// notify_cursors for the given batch. Delegates to preloadExistingKeys.
func preloadExistingNotifyCursorIDs(ctx context.Context, tx *sql.Tx, recs []*NotifyCursorRecord) (map[string]bool, error) {
	return preloadExistingKeys(ctx, tx, "notify_cursors", "id", recs, func(r *NotifyCursorRecord) string { return r.ID })
}
