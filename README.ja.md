...
AI コーディング CLI（Claude Code, Codex, Grok Build）をモバイルからリモート操作し、複数台のマシンを跨いで記憶・思考・協調する永続 AI エージェントを運用するツール。
...
- 対応 CLI: `claude`, `codex`, `grok`（Grok Build）— いずれか 1 つ以上
...
2 台目以降のマシンを Hub に参加させる手順は 1 コマンドです。
macOS / Linux / Windows を任意に組み合わせて構成できます。
...
Hub を自動発見し、Hub を自分の peer registry に書き込んで join
request を POST します。Hub は request を保留し、Settings → Peers →
「Pending join requests」に表示します。Owner が **Approve** を押すと
peer に privileged surface (sessions / files / git) を許可します。
Reject なら request は削除されるだけで、peer は再要求できます。
...
peer のリスナーを Tailscale インターフェース IP のみに bind し、
`0.0.0.0` フォールバックを禁止する場合:

```bash
kojo --peer --tailnet-only
```

従来の手動ペアリング (`--peer-add` / `--peer-trust` / `--peer-remove`)
は escape hatch として残してあります。

マルチデバイス構成の全手順、agent の device-switch、Hub 引っ越しは
[docs/multi-device.ja.md](docs/multi-device.ja.md) を参照してください。

### 旧リリースからのアップグレード

v0.101.0 で on-disk の設定ディレクトリ構成が変わりました。旧リリース
(`~/.config/kojo/` または `%APPDATA%\kojo\` を使う v0.x 系) から
上げる場合は [docs/migration.ja.md](docs/migration.ja.md) を参照して
ください。`--migrate` の手順、ロールバック、引き継がない機能
(Gemini CLI backend と Gmail 通知は削除) を解説しています。
...
- セッション再起動（ツール固有の resume: `claude --resume`, `codex resume`, `grok --resume`）
...
- Slack 連携（Socket Mode）— エージェントごとの Bot、ライブストリーミング応答、スレッド単位のセッション再開、チャンネル / スレッドを認識するシステムプロンプト
...
| エージェント | Claude / Codex / Custom API バックエンド、暗号化 SQLite（クレデンシャル + FTS5 記憶） |
| 通知 | Web Push (VAPID) |
