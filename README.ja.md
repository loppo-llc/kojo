...
| エージェント | Claude / Codex / Gemini バックエンド、暗号化 SQLite（クレデンシャル + FTS5 記憶） |
| 通知 | Web Push (VAPID) |
