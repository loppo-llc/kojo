  ...
  return (
    <div className="flex flex-col h-full bg-app text-ink">
      ...
      <header className="sticky top-0 z-40 flex h-[52px] shrink-0 items-center gap-2 border-b border-hairline bg-app/85 px-2 backdrop-blur sm:px-3">
        ...
        {isThread && group.members[0] && (
          <button
            // Pushed (not replaced): the browser's root "back" is
            // navigate(-1), which must land back on this thread (or draft).
            onClick={() => navigate(`/agents/${group.members[0].agentId}/data`, {
