...
| `--migrate-external-cli=true/false` | claude の transcript link 移行。default `true`。modifier flag なので primary mode と併用 |
...
# 2. migrate (v0 dir は手元 backup ツール (Time Machine 等) で別途固めておく)
kojo --migrate
