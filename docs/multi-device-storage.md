...
| `vapid.json` (public part) | `kv` (namespace=`notify`, key=`vapid_public`) | global |
| `vapid.json` (private key) | `kv` (namespace=`notify`, key=`vapid_private`, secret=true, envelope encrypted) | machine (KEK は host-bound、`<v1>/auth/kek.bin`、3.4 節参照) ※将来 cluster-bound 化を検討 |
| `auth/owner.token` (生値) | (migration 時は **import せず v1 で新規生成** = 5.5 step 8。v1 通常運用では Phase 2c-2 slice 17 により `kv` (namespace=`auth`, key=`owner.token`, scope=`global`, type=`string`, value=`sha256:<hex>`) に sha256 hash を保存。design 表の "bcrypt hash" 記述は将来の algorithm 変更を見据えた表記、現行は sha256 を維持) | global (hash) |
| `auth/agent_tokens/<id>` (生値) | 同上 (kv key は `agent_tokens/<id>`)。`<configdir>/auth/` の disk file は pre-cutover binary との rollback 互換 fallback として残置、kv 書き込み成功後に best-effort unlink される | 同上 |

### 2.4 ブロブ (native blob API)

| 旧パス | 新 URI | scope |
|-------|--------|-------|
| `agents/<id>/avatar.{png,svg,jpg,jpeg,webp}` | `kojo://global/agents/<id>/avatar.<ext>` | global (runtime probe順は `internal/agent/avatar.go` の `avatarExtProbe`、importer の許容拡張子は `internal/migrate/importers/blobs.go` と一致) |
...
claude / codex など外部 CLI は kojo を介さず直接 `MEMORY.md` を書く。これを DB 正本化するには 3 案:
...
- **owner / agent token の生値はどこにも保存しない**
  ...
  - **Hub DB に hash を保存** = global 正本。Hub に集約することで全 peer が同じ hash 集合で検証可能
    - Phase 2c-2 slice 17 の現行実装: `kv` (namespace=`auth`, key=`owner.token` / `agent_tokens/<id>`, scope=`global`, type=`string`, value=`sha256:<hex>` strict-lowercase) に sha256 hash を保存
    - 将来の algorithm agility (bcrypt / scrypt / argon2 への切替) は別 slice。同一 namespace + 同一 key で value prefix を `bcrypt:` / `argon2id:` 等に切替えるが、parseAuthKVValue の row-shape gate 拡張だけでは不十分:
      - 現行は token を SHA-256 (固定 digest) して hash 値そのものを逆引き map のキーにしているため `LookupAgent` は O(1) map lookup で済む。bcrypt/scrypt/argon2 は **salt 付き** で同じ token でも毎回違う digest になり、map による reverse lookup が成立しない
      - 切替時の代替設計: (a) row に salt と一緒に短い non-secret な `lookup_key` (例: HMAC-SHA256(constant_key, raw_token) の最初の 16 bytes) を併記して逆引き map のキーとして使い、map hit 後に bcrypt/argon2 verifier で final check、または (b) token に id-prefix (例: `tok_<8hex>_<random>`) を付けて prefix から row を直接引く、いずれかが必要
      - `hashToken` / `VerifyOwner` / `LookupAgent` / 内部 reverse-lookup map の algorithm 分岐 (presented をどう正規化して row を引き、どう verify するか) を導入することになる
  - 各 peer は Hub からこの hash 集合を pull (read-through cache)。Bearer 受信時に local hash と比較
  - 旧 `auth/owner.token` `auth/agent_tokens/*` は migration 時 **import せず**、v1 で新規 token を生成 → Web UI / agent CLI に再配布要求 (5.5 step 8)。v0 の生値ファイルは v1 から書き換えず、v0 dir 削除まで残る (slice 29 の `kojo --clean v0 --clean-apply` で soft-delete、または手動 `rm -rf`)
- WebDAV 用に **別の短命 token** を発行 (scope 限定、TTL 数時間、UI から発行/失効可能)
- VAPID **private** key の扱い:
  - v1 (現状の実装): kv (`namespace=notify`, `key=vapid_private`, `secret=true`, `scope=machine`) に envelope 暗号化で保存
    - データ鍵 (DEK): VAPID private を AES-GCM で暗号化
    - 鍵暗号化鍵 (KEK): `<v1>/auth/kek.bin` (mode 0600) に **host-bound** で保管 — peer ごとに独自 KEK
    - AAD は `notify/vapid_private` で row slot に bind
    - 公開鍵は kv (`key=vapid_public`, `scope=global`, 平文)
  - v1 → v2 で **cluster-bound KEK 化** を予定 (failover 時の復号継続性のため。下記参照):
    - Hub と backup peer に同じ KEK を pre-shared
    - Hub snapshot (3.6 節) に暗号化済み private + KEK 識別子を含める形に変更
    - 現状の host-bound KEK では新 Hub が暗号化済 private を復号できないため、backup peer は failover 後に新 VAPID pair を再生成し、既存の web push 購読は再認可が必要になる
  - KEK のローテーションは v2 以降
...
- **Optimistic lock**: 全 write は `If-Match: <etag>` を要求。RFC 7232 に従い If-Match の precondition 失敗は **412 Precondition Failed** で返す。レスポンスボディに現状 etag と current record snapshot を含める → UI で diff/merge。If-Match 未指定で write が来た場合は **428 Precondition Required** を返す方針 (v1 移行期は best-effort で許可、client 移行完了後に 428 を有効化)
...
5. 該当 agent の PTY セッション (local scope) は失われる
   ...
   - codex も同等の resume 機構 / kojo session ID 維持
...
| KEK loss (Hub と backup peer 両方) | secret 復号失敗 | envelope 暗号化された secret rows のみ全再生成: VAPID private 鍵再生成 (push subscription 全失効、再購読要求)、その他 `secret=true` の kv 行全削除 + 再生成。owner / agent token hash は `kv auth/*` に non-secret (`secret=false`) で保存されているため KEK 不要で検証可能、KEK loss では rotation 不要 (運用方針として併せて rotate するのは任意) |
...
| Migration | v0 → v1 import 完走、import 中の kill -9、再実行で resume、v0 dir read-only 保証、現行 `--clean snapshots\|legacy\|all` が v0 dir を touch しないこと、`--clean v0` (slice 29) が v1 dir を touch しないこと、`--clean v0` の manifest divergence guard が `--clean-force` 無しで refuse すること |
...
- v0 dir の削除は **ユーザーが明示的に** 行う (`kojo --clean v0 --clean-apply` で soft-delete、または手動 `rm -rf`、slice 29)。kojo は自動削除しない
...
| `sessions.json` | `sessions` table (local scope、peer_id 付与、**status は全て `archived` に強制**) ／ **runtime tmux 再 attach は別レーン**: `internal/session/store.go` の Load() が kv miss 時に v1 dir → v0 dir の順で legacy `sessions.json` を読み、TmuxSessionName を保持したまま kv namespace=`sessions` に mirror する。`kojo_<id>` tmux 名と `tmux attach-session` 形式は v0/v1 wire-compatible なので、v0 が残した live tmux pane に v1 起動後そのまま再 attach できる。v0 dir 側の `sessions.json` は **削除しない** (rollback 用 + `kojo --clean v0` の領分) |
...
| `auth/owner.token` | (import せず、v1 で新規生成。v0 file 自体は v0 dir 削除まで残る (`kojo --clean v0 --clean-apply` で soft-delete 可能、slice 29)) |
...
7. blob (avatar, books, outbox, temp, index/memory.db) を v1 blob store にコピー、`blob_refs` 登録
   ...
   - `.cron_last` (v0 throttle marker) は **import しない**: cron throttle は §2.3 表に従い `kv` (namespace=`scheduler`, key=`cron_last/<agentID>`, scope=`machine`) に移行済。50秒窓の dotfile mtime を移行する価値がないので runtime 起動後に best-effort unlink するだけ (詳細は `internal/agent/cron_lock_kv.go` removeLegacyCronLock コメント)
   - **コピー時の制約**:
...
9. **post-import manifest verify**: v0 dir の sha256 manifest を **再計算** し、step 3 で記録した値と一致するか確認
    - 不一致 → import 中に v0 が外部から書き換えられた → migration を **fail** させる
    - v1 dir はそのまま残す (`--migrate-restart` で再実行可能)
    - `migration_complete.json` は作成しない
10. 一致した場合のみ `migration_in_progress.lock` を `migration_complete.json` に rename。中身:
    ```json
    {
      "v0_path": "...",
      "v0_sha256_manifest": "...",
    ...
    ```
    起動時の self-integrity check は `migration_complete.json` の必須フィールド (`v1_schema_version`, `migrator_version`, `completed_at`) が well-formed であること + kojo.db の schema_version が `v1_schema_version` 以上かつバイナリ最大サポート以下であること、のみ。`v0_sha256_manifest` は `kojo --clean v0` の `--clean-force` gate (5.8) でのみ参照し、boot では再計算しない (詳細は 5.9)
11. 起動メッセージに「migration 完了。v0 data は <path> に残っています。restart kojo without --migrate to begin normal v1 operation。 v0 rollback が不要になったら `kojo --clean v0 --clean-apply` で soft-delete (Phase 2c-2 slice 29、`kojo.deleted-<ts>/` への rename、物理削除は operator-driven)」を表示

### 5.5.1 外部 CLI transcript dir の継続性

各 agent backend (`internal/agent/backend_{claude,codex}.go`) は CWD を agent dir に設定する:
...
#### 5.5.1.3 hash 関数 / config dir resolver の出所
...
codex は path-hash mapping 不要 (CLI 側の transcript layout が異なる)。external-CLI 互換層は claude のみ。判明しない CLI については migration で skip → fresh session で許容。

#### 5.5.1.4 制約と非ゴール

- 旧 v0 transcript store (claude/codex の v0 hash dir) 自体は **kojo は touch しない** (外部 CLI 管轄)。ユーザーが各 CLI の clean 機能で削除する
- 「外部 CLI transcript の取り込みは非ゴール」(8 節) は変えない。symlink は**継続性のためだけ**の薄い互換 layer
- v1 から v1.x で agent dir path 変更が起きる際 (例えば configdir のさらなる変更) は同様の問題が再発する。その時もこの仕組みを再利用する

#### 5.5.1.5 v0 rollback 時の transcript 混在リスク

v1 で symlink を作ったあと v0 binary に rollback すると:

- v1 で claude / codex が更新した transcript は **v0 と共通の transcript store 実体** に書かれている (symlink target 経由)
...
- claude / codex どちらも実体共有 = rollback 時の混在リスクは同等

**rollback の前提として**:

- ユーザーが v0 binary を起動する場合、**`--migrate-external-cli` の効果も巻き戻す手順** を想定する:
  - claude / codex: v1 hash dir の symlink を `os.Remove` で削除 (target 実体は触らない)
...
- 「auth 生値破棄」とは「v1 dir に持ち込まない」の意味。v0 file 自体は v0 dir 削除まで残る (`kojo --clean v0 --clean-apply` で soft-delete 可能、slice 29)
...
1. v1 binary を停止
2. **`kojo --rollback-external-cli` を v1 binary で実行** (5.5.1.5 参照):
...
外部 CLI transcript の v1 で増えた turn は **v0 / v1 共通の transcript store** (claude / codex は symlink target) に物理的に残るが、v0 binary が読めるかは v0 / v1 の format 互換性次第。

### 5.8 `--clean v0` v0-dir cleanup target (**Phase 2c-2 slice 29 + slice 30: soft-delete + trash purge landed**)

> **ステータス**: Phase 2c-2 slice 29 で **soft-delete only** の最小実装が
> landed (`cmd/kojo/clean_v0.go`)。`kojo --clean v0` は migration 完了済の
> v0 dir を sibling の `~/.config/kojo.deleted-<ts>/` (Windows は
> `%APPDATA%\kojo.deleted-<ts>\`) に rename する。dry-run 既定。
>
> **実装済み**:
>
> - `--clean v0` target (target='v0', slice 29) — v0 dir の soft-delete (rename to sibling)
> - `--clean-force` flag (slice 29) — **manifest 不一致のみ** override 可能 (v0 が migration 後に編集されている場合の唯一の force-able 状態)
> - `--clean v0-trash` target (target='v0-trash', slice 30) — `kojo.deleted-<ts>/` siblings を物理削除。dir 名の stamp で age filter (`--clean-min-age-days`、CLI default 7、`=0` で全 age purge — soft-delete recovery window を defeat するので explicit 要)。anomalies (non-dir / symlink / 解析不能 stamp / numeric overflow) は untouched。future-dated stamp は age cutoff の後ろなので KeepYoung に分類し、apply 直前の future guard でも reject。各 entry を apply 直前に re-Lstat + name + stamp 同一性 + future guard 検証 (scan→apply window でのすり替え防御)
> - 安全 gate (順番に check、最初の block を PartialReason として report):
>   1. v0 dir 不在 → skip (no-op)
>   2. v0 path が symlink → 拒否 (force-able **ではない**。symlink を rename すると link 自体が動き target は残る — operator 意図不明の確率が高い)
>   3. v0 path が dir でない (regular file 等) → 拒否 (force-able **ではない**)
>   4. v0 binary が `kojo.lock` を保持 (configdir.Probe で検出) → 拒否 (force-able **ではない**。running binary の下で rename すると open file 破損)
>   5. v1 dir に `migration_complete.json` 不在 / parse 失敗 / `v0_sha256_manifest` 空 → 拒否 (force-able **ではない**。migration 履歴が証明できない)
>   6. `migration_complete.json` の `v0_path` が cleanup target と不一致 → 拒否 (force-able **ではない**。複数 config dir の混同を catch。空フィールドは older / malformed `migration_complete.json` 互換のため tolerate — manifest gate が二重防御)
>   7. v0 manifest を再計算し `migration_complete.json` の manifest と比較 → 不一致時 **`--clean-force` 必須**
>   8. trash path に既存 dir がある → clobber 拒否 (force-able **ではない**)
>   9. apply 時に full re-plan — v0 path / symlink / non-dir / lock held / `migration_complete.json` / manifest をすべて再検証する。新たな non-forceable block があれば abort。manifest divergence は `--clean-force` 指定時のみ通過 (未指定なら同じく abort)。trash collision check も rename 直前に再実行 (probe-then-act window を最小化)
> - `target='all'` には **入れていない**: v0 dir は唯一 destructive な rollback fallback target なので、定期的な snapshot/legacy 掃除で誤って消さないよう explicit `--clean v0` のみで実行
>
> **物理削除**: soft-delete 後の `kojo.deleted-<ts>/` は slice 30 の
> `kojo --clean v0-trash --clean-apply` で operator が明示的に物理削除する
> (default `--clean-min-age-days=7` で recovery window 7 日を維持、`=0` で全 age)。
> v1 binary が起動時に自動 sweep することはしない — operator が cron で
> `kojo --clean v0-trash --clean-apply --clean-min-age-days=7` を回す運用想定。
> slice 29 の apply 完了時に stderr に `kojo: physical removal:
> \`kojo --clean v0-trash --clean-apply\` (slice 30;
> default --clean-min-age-days=7) — or \`rm -rf\` if you skip the recovery
> window` と案内する。

```
kojo --clean v0                          # dry-run: rename 計画と manifest 一致状況を表示
kojo --clean v0 --clean-apply            # apply: soft-delete を実行
kojo --clean v0 --clean-apply --clean-force  # apply: manifest 不一致でも override

kojo --clean v0-trash                                       # dry-run: kojo.deleted-<ts>/ 一覧 (slice 30、default --clean-min-age-days=7)
kojo --clean v0-trash --clean-apply                         # apply: 7 日経過の trash dir を物理削除 (default)
kojo --clean v0-trash --clean-apply --clean-min-age-days=0  # apply: **全 age** の trash dir を即時削除 (recovery window を defeat)
```

実装規約 (slice 29 で固定済 + 将来 slice の制約):

- 削除対象の path は **v0 dir に限定** (configdir.V0Path() の戻り値、または `v0PathOverride()` テスト用フック)、v1 dir は絶対に touch しない (`runV0` は `target=='v0'` のみ true)
- 削除前に v0 dir の現 sha256 manifest (`migrate.ManifestSHA256`) を再計算 → `migration_complete.json` 内の manifest と比較
  - 一致 → そのまま soft-delete に進める
  - 不一致 → PartialReason に divergence を記録、apply で refuse、operator が `--clean-force` で override
- **soft-delete (現状)**: `os.Rename(v0, <home>/.config/kojo.deleted-<ts>/)`。同一 filesystem 上の sibling rename を前提とする (POSIX は atomic)。Windows は MoveFileEx 系で同 volume rename なら成功するが open file handle (旧 v0 binary が dir を open している等) があれば EBUSY 系で失敗し得る — 失敗時は v0 dir はそのまま残り、operator が原因を解消して再 run する
- 物理削除は **operator 手動** (slice 29 では未自動化)
- 将来 slice で `--clean-force` の wording を「migration 完了後に v0 が変更されています。本当に削除するか?」プロンプト形式に拡張可能 (現状は flag だけで通る、operator 自身が CLI で run する想定なので prompt 不要)

### 5.9 起動時の常時表示と manifest verify

v1 起動時:

- v0 dir 残存 + `migration_complete.json` あり → **通常起動**。UI ヘッダーに `v0 data is still present (<size> GB). If you no longer plan to roll back to a v0 binary, run \`kojo --clean v0 --clean-apply\` to soft-delete it (Phase 2c-2 slice 29).` を表示。v1.1 リリース後でも残存しているユーザーには alert color。UI を持たない CLI / headless 起動時は同等の警告を stderr / logger に出力する
- v0 dir 不在 (= clean 済) + `migration_complete.json` あり → 通常起動
- v0 dir 残存 + `migration_complete.json` なし → migration 未実行扱い (5.3 trigger に従う)
- v0 dir 不在 + `migration_complete.json` なし → 新規セットアップ

`migration_complete.json` 自体の self-integrity は schema_version + migrator_version + completed_at が well-formed であること、および kojo.db の schema_version が `migration_complete.json` の記録値以上 / バイナリ最大サポート値以下であることだけ check。**v0 manifest を boot で再計算することは行わない** — 起動して v1 がカノニカルである以上、v0 dir の事後改変を boot で refuse する根拠は弱く、`migration_complete.json` 内 `v0_sha256_manifest` は専ら `kojo --clean v0` 側の `--clean-force` gate (5.8) でのみ参照する。

**自動削除はしない**。v0 dir 削除は完全にユーザー意思 (`kojo --clean v0 --clean-apply` で soft-delete、または手動 `rm -rf`、slice 29)。

### 5.10 v0 が外部書き換えされている場合

- claude / codex が v0 dir に直書きしている最中に migration 開始 → 部分 import の危険
...
| `cmd/kojo/main.go` | `--migrate`, `--migrate-restart`, `--fresh`, `--migrate-external-cli` (default on), `--rollback-external-cli` フラグ |
| `cmd/kojo/clean_v0.go`, `cmd/kojo/clean_v0_trash.go` | Phase 2c-2 slice 29 / 30 で landed。`--clean v0` target が v0 dir を `~/.config/kojo.deleted-<ts>/` に rename (soft-delete)。`--clean v0-trash` target (slice 30) が `kojo.deleted-<ts>/` siblings を物理削除 (`--clean-min-age-days` で age filter、anomalies は untouched)。`migration_complete.json` の v0 manifest と現状を比較し divergence あれば `--clean-force` 必須。`target='all'` には **意図的に含めない** (v0 / trash どちらも recovery window、誤削除防止) |
| `internal/store/` | schema 適用 + `migration_status` テーブル管理 |
| `internal/agent/backend_*.go` | claude / codex は path-hash 関数 (claude は既存の `claudeEncodePath`、codex は実装時に確認・追加) を、migration が呼び出せる形で expose |
...
- `kojo --clean v0` (slice 29、`--clean-apply` 無しで dry-run) を v0 dir 残存環境で run → trash path candidate のみ表示され v1 dir は触られない (`--clean v0 --clean-apply` でも v1 dir は無関係)。manifest divergence + `--clean-force` の override も検証する
...
- `kojo --rollback-external-cli` 実行で claude / codex の v1 symlink が削除される (target は残存)
...
6. external CLI (claude / codex) の transcript / project state を kojo に取り込むか、別 sync 戦略 (Syncthing) で扱うか
...
| `internal/auth/store.go` | token hash は kv (namespace=`auth`, scope=global) に保持、Phase 2c-2 slice 17 で disk → kv 移行。raw token は in-memory のみ (post-issue は再表示不可) |
| `internal/notify/webpush.go` | subscription / cursor / vapid public は kv table、private は machine-scope kv (envelope encrypted, host-bound KEK at `<v1>/auth/kek.bin`)。cluster-bound KEK は v2 で計画 (3.4 節参照) |
| `internal/session/store.go` | v0→v1 importer は Phase 2c-1 slice 7 で landed (`sessions` table、status=archived 強制)。runtime cutover は Phase 2c-2 slice 28 で完了 — kv (namespace=`sessions`, key=`all`, scope=`local`, type=`json`) に SessionInfo の list を JSON blob で保持。legacy `<configdir>/sessions.json` は first Load で kv mirror + best-effort unlink される (cron_paused / loadOrCreateOwner と同 pattern)。v1 dir に legacy が無い場合は **v0 dir の `sessions.json` を読み込んで kv に mirror** する (TmuxSessionName 含む全 SessionInfo を保持、v0 dir 側ファイルは unlink しない: rollback 用と `kojo --clean v0` 領分のため)。v0 fallback は startup gate で `v1Complete && v0Exists` を確認した起動でのみ有効化 (`server.Config.V0LegacyDir`)、`--fresh` 起動では明示 opt-out。これで v0 が残した live tmux pane を v1 が `restoreSession → tryReattachPersistedTmux` 経由で拾えて、`cleanupOrphanedTmuxSessions` の known set にも入る。`parseAndFilter` の `maxAge` cutoff (7d) は `Status=running && TmuxSessionName != ""` の row を例外で残す (cutoff で落とすと known set から外れて v1 起動時に kill される)。importer が書く `sessions` table の archived rows は別 storage site に置かれており現状の runtime は consult しない (将来 sessions table を runtime canonical 化する slice で activate 予定、status vocabulary 'exited' vs 'stopped/archived' の reconcile もそこで対応) |
...
| `cmd/kojo/main.go` | `--migrate`, `--migrate-restart`, `--fresh`, `--migrate-external-cli`, `--rollback-external-cli`, `--snapshot`, `--clean` 系フラグ (詳細は `flag.Parse` 直前のブロック参照) |
| `cmd/kojo/clean_cmd.go`, `cmd/kojo/clean_legacy.go`, `cmd/kojo/clean_v0.go`, `cmd/kojo/clean_v0_trash.go` | `--clean` flag の処理本体。target = `snapshots` (古い snapshot dir GC) / `legacy` (Phase 2c-2 で kv 化が済んだ legacy disk file を sweep。`.cron_last` のように value を kv に持ち込まず unlink-only な物も含む。**安全条件**: 対応する canonical kv 行が存在し、かつ runtime の row-shape / value validator を pass すること。scan→apply の race も apply 直前に再検証する。slice 19 で追加) / `v0` (slice 29 で追加。v0 dir を sibling の `kojo.deleted-<ts>/` に soft-delete。manifest divergence は `--clean-force` で override) / `v0-trash` (slice 30 で追加。`kojo.deleted-<ts>/` を `--clean-min-age-days` で age filter してから物理削除) / `all` (snapshots + legacy のみ。v0 / v0-trash は意図的に除外) |

## 8. 非ゴール

- claude / codex の transcript / project state の取り込み
