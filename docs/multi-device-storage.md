...
12. 起動メッセージに「migration 完了。v0 data は <path> に残っています。restart kojo without --migrate to begin normal v1 operation。 v0 rollback が不要になったら `kojo --clean v0 --clean-apply` で soft-delete (Phase 2c-2 slice 29、`kojo.deleted-<ts>/` への rename、物理削除は operator-driven)」を表示
...
> **未実装** (将来 slice):
>
> - `--keep-blobs`: DB / config 系のみ削除、`books/` / `temp/` blob を残す
> - `--hard-delete`: soft-delete を経ず即時 `os.RemoveAll`
> - 7 日後の自動物理削除 (起動時 sweep)。slice 30 の `kojo --clean v0-trash --clean-apply --clean-min-age-days=7` を operator が cron 等で自動化することで暫定対応可能
>
> **物理削除の現状**: soft-delete 後の `kojo.deleted-<ts>/` は slice 30 の
> `kojo --clean v0-trash --clean-apply` で operator が明示的に物理削除できる
> (default `--clean-min-age-days=7` で recovery window 7 日を維持、`=0` で全 age)。
> v1 binary が起動時に自動 sweep することは **しない** (7-day auto-sweep は
> 将来 slice; operator が cron で `kojo --clean v0-trash --clean-apply
> --clean-min-age-days=7` を回すことで暫定対応)。slice 29 の apply 完了時に
> stderr に `kojo: physical removal: \`kojo --clean v0-trash --clean-apply\`
> (slice 30; default --clean-min-age-days=7) — or \`rm -rf\` if you skip the
> recovery window` と案内する。
...
# 未実装 (将来 slice):
# kojo --clean v0 --hard-delete   # soft-delete を skip して物理削除
```
...
| `cmd/kojo/main.go` | `--migrate`, `--migrate-restart`, `--fresh`, `--migrate-external-cli`, `--migrate-backup`, `--rollback-external-cli`, `--snapshot`, `--clean` 系フラグ (詳細は `flag.Parse` 直前のブロック参照) |
